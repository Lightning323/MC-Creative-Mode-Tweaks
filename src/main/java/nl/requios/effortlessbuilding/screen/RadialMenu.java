...
public class RadialMenu extends Screen {
    ...
    private void drawSideButtonBackgrounds(BufferBuilder buffer, double middleX, double middleY, double mouseXCenter, double mouseYCenter, ArrayList<MenuButton> buttons, double scale) {
        for (MenuButton btn : buttons) {
            ...
            boolean isSelected = btn.enabled && (btn.action == ModeOptions.getBuildSpeed() || btn.action == ModeOptions.getFill() || btn.action == ModeOptions.getCubeFill() || btn.action == ModeOptions.getRaisedEdge() || btn.action == ModeOptions.getLineThickness() || btn.action == ModeOptions.getCircleStart() || btn.action == ModeOptions.getPointBuild() || btn.action == ModeOptions.ActionEnum.CYCLE_REPLACE_MODE && BuildSettings.CLIENT.getReplaceMode() != BuildSettings.ReplaceMode.ONLY_AIR || btn.action == ModeOptions.ActionEnum.TOGGLE_ANGEL_PLACEMENT && BuildSettings.CLIENT.isAngelPlacementEnabled());
            Vector4f color;
            if (!btn.enabled) {
