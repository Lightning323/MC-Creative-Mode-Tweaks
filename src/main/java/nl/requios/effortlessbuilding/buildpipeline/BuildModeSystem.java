...
public class BuildModeSystem implements IBuildSystem {
   ...
   public void processBlocks(BlockSet blocks, Player player, BuildPipeline.BuildState action) {
      ...
      if (ctx != null) {
         ModeOptions.applyForCalculation(ctx.fill(), ctx.cubeFill(), ctx.raisedEdge(), ctx.circleStart(), ctx.pointBuild(), ctx.sides(), ctx.planeAlign());
         ctx.mode().instance.setFirstClickFace(ctx.firstClickFace());
         List<BlockPos> rawPositions = ctx.mode().instance.getPlacementBlocks(player, ctx.firstPos(), ctx.secondPos(), ctx.thirdPos(), ctx.fourthPos());
